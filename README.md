...
[![Quality](https://github.com/nsjjjjjj/KNU-Ask/actions/workflows/quality.yml/badge.svg)](https://github.com/nsjjjjjj/KNU-Ask/actions/workflows/quality.yml)
...
| 문제 | 해결 | 확인 결과 |
|---|---|---|
| 모든 공지를 AI로 미리 처리하면 비용과 대기열이 커짐 | exact/canonical 캐시와 공식 사이트 확인을 결합한 온디맨드 파이프라인 | 실제 질문에 필요한 근거만 추가 처리하고 기존 일괄 워커는 롤백용으로 유지 |
| 공지는 찾았지만 절차·서류·연락처가 구조화되지 않음 | 요청 필드만 원문·첨부에서 지연 복구하고 인용문·도메인·OCR 신뢰도 검증 | warm 요청에서 PDF·OCR·Codex 재호출 없이 저장 결과 재사용 |
| 하위 탭 누락, 마지막 페이지 반복, NUL 저장 실패 | 출처 프로필, 페이지 지문, 재귀 정제, 파일 바이트 판별 | 전체 수집의 원문 최종 실패 0건 |
| 종료·충돌·근거 부족을 같은 답변 형태로 표시 | 상태별 안내, 다음 행동, 담당 부서, 공식 근거를 구조화 | 정상·마감·충돌·근거 부족·오류 흐름을 각각 테스트 |

상세한 문제·원인·해결·검증 과정은 [개발 문제 해결 기록](docs/development-log.md)과 [운영 검증 보고서](docs/reviews/operational-validation-2026-07-21.md)에 정리했습니다.
...
브라우저에서 `http://localhost:8080`을 엽니다. 로컬 관리자 예상량 화면은 `http://localhost:8080/admin`입니다. 기본 설정은 샘플 크롤러를 사용하며, AI 키가 없거나 지정한 공급자를 사용할 수 없으면 규칙 기반 응답으로 안전하게 폴백합니다.
...
실제 키와 토큰은 `.env`에만 저장하고 Git에 커밋하지 않습니다. `ADMIN_API_TOKEN`은 32자 이상의 임의값으로 바꿔야 하며, 비어 있거나 예시값이면 관리자 API가 자동으로 비활성화됩니다. 실사이트 크롤링 결과·운영 체크포인트·첨부 캐시도 Git에서 제외하고, 저장소에는 재현 가능한 샘플과 검증 문서만 유지합니다. 커밋 전에는 다음 검사를 실행합니다.
...
| `CODEX_ENRICHMENT_ENABLED` | `false` | 전체 문서 일괄 enrichment 큐 수신. 명시적으로 켜기 전에는 비활성화 |
| `ON_DEMAND_SEARCH_ENABLED` | `true` | exact/canonical 캐시와 질문 시점 근거 검색 사용. `false`이면 기존 검색 흐름으로 롤백 |
| `ON_DEMAND_LIVE_SEARCH_ENABLED` | `true` | 로컬 DB 근거가 부족할 때 공식 홈페이지를 최대 2회·6 URL·30초 범위로 확인 |
| `ON_DEMAND_CODEX_ENABLED` | `true` | canonical 캐시 미스이면서 근거가 부족할 때만 Codex 근거 판정 사용 |
| `ON_DEMAND_CODEX_PROVIDER` | `codex_exec` | 온디맨드 근거 판정도 Mac 호스트의 로그인된 Codex CLI 작업 큐로 처리 |
| `ON_DEMAND_ALLOWED_DOMAINS` | `kangnam.ac.kr,web.kangnam.ac.kr` | 실시간 검색이 허용하는 공식 학교 도메인 |
| `MISSING_EVIDENCE_RECOVERY_ENABLED` | `true` | 사용자가 요구한 절차·서류·연락처·신청 링크가 구조화 DB에 없을 때만 원문과 첨부를 지연 재확인 |
| `MISSING_EVIDENCE_RECOVERY_TIMEOUT_SECONDS` | `12` | 지연 재확인 요청의 제한 시간 |
| `MISSING_EVIDENCE_NEGATIVE_TTL_SECONDS` | `3600` | 공식 자료에서 확인되지 않은 결과를 다시 검사하기 전의 짧은 TTL |
| `MISSING_EVIDENCE_MAX_PDF_PAGES` | `20` | 한 번의 지연 재확인에서 읽거나 OCR할 PDF 최대 페이지 수 |
| `LLM_CANDIDATE_RERANKING_ENABLED` | `false` | 별도 LLM 후보 재정렬. 기본 비활성화 |
...
| `EMBEDDING_PROVIDER` | `ollama` | 로컬 BGE-M3, 사용할 수 없으면 무토큰 어휘 폴백. 크롤링 중 외부 임베딩 토큰을 쓰지 않음 |
...
| `CRAWLER_EVENT_DETAIL_LIMIT` | `300` | 행사 본문은 모두 저장하되 신청형 첨부 추출·OCR을 우선할 최대 건수 |
| `CRAWLER_EXTRACT_ATTACHMENTS` | `true` | 첨부파일·본문 이미지 텍스트 추출 및 OCR |
| `CRAWLER_ATTACHMENT_CACHE_TTL_HOURS` | `24` | ETag가 없는 성공 첨부를 다시 내려받기 전 캐시 유지 시간 |
| `CRAWLER_ATTACHMENT_CACHE_DIR` | `/app/data/attachment-cache` | Compose 볼륨 경로. 코드의 로컬 기본값은 `/tmp/knuask-attachment-cache` |
| `CRAWLER_SCHEDULE_MINUTES` | `60` | 자동 증분 수집 주기 |
| `CRAWLER_FULL_SCHEDULE_DAY` | `sun` | 전체 공개 자료의 변경을 확인할 요일 |
| `CRAWLER_FULL_SCHEDULE_HOUR` | `3` | 주간 전체 범위를 다시 확인할 시각 |
| `ADMIN_API_TOKEN` | 설정 필수 | 32자 이상의 임의값. 크롤러·재처리 API 보호용 관리자 토큰 |
...
관리자 미리보기에서 범위와 AI 작업 상한을 확인한 뒤 단계별 모드로 시작합니다.

```bash
curl -H "X-Admin-Token: $ADMIN_API_TOKEN" 'http://localhost:8080/api/crawler/preview?mode=pilot'
curl -X POST -H "X-Admin-Token: $ADMIN_API_TOKEN" 'http://localhost:8080/api/crawler/run?mode=pilot'
curl -H "X-Admin-Token: $ADMIN_API_TOKEN" http://localhost:8080/api/crawler/status
```

`mode`는 `incremental`, `daily`, `pilot`, `full` 중 하나입니다. `pilot`은 최근 1개월 행사, 대표 학사안내·FAQ·장학·2024~2026 대학요람·취업 프로그램·기숙사·교환학생·전화번호부만 처리합니다. 검증 후 사용하는 `full`은 최근 12개월 공지와 행사, 전체 학사/FAQ, 공개 현행 규정, 학생지원 정적 안내와 하위 사이트 공지까지 확장합니다. 사이트 개편 시 `backend/app/services/crawler/knu.py`의 선택자를 조정해야 합니다.
...
기본 운영은 온디맨드 방식입니다. 전체 공지의 본문을 Codex로 미리 처리하지 않고 URL·제목·게시일·첨부 URL·원문 해시를 유지한 뒤, 캐시에 없는 실제 질문에서만 Gemini QueryPlan과 근거가 부족한 경우의 Codex 판정을 사용합니다. exact cache는 Gemini 전에, canonical cache는 QueryPlan 직후에 확인합니다. 온디맨드 판정도 최초 전처리와 같은 Mac 호스트의 `codex exec` 워커가 수행하며 OpenAI API 키를 사용하지 않습니다. Codex에는 사용자 원문을 보내지 않고 QueryPlan과 허용 도메인에서 내려받아 정제한 자료만 전달합니다.

기존 일괄 워커는 롤백·수동 복구용으로 남아 있으며 `CODEX_ENRICHMENT_ENABLED=false`가 기본입니다. 다시 켤 때만 `NOTICE_STRUCTURING_PROVIDER=codex`와 함께 사용합니다. 플래그를 끄면 worker의 새 작업 claim이 중단되고 pending/running 큐 레코드는 삭제하지 않습니다.

LaunchAgent와 Git에는 관리자 토큰을 저장하지 않습니다. 설치 스크립트가 프로젝트 `.env`에서 토큰 하나만 읽어 macOS 개인정보 보호 경로 밖의 권한 `600` 파일로 옮기며, plist에는 그 파일 경로만 기록합니다. 워커나 plist를 갱신한 뒤에는 다시 실행합니다.

```bash
python3 scripts/install_codex_launch_agent.py
```
...
검증된 답변 캐시는 질문 원문 대신 정규화 질문 해시, canonical 조건 키, QueryPlan, 사실, 출처 URL·제목·실제 인용문·원문 해시와 만료 시각을 저장합니다. 일정은 짧게, 규정·졸업요건은 길게 유지하며 원문 해시가 바뀐 자료는 다시 확인합니다. 별도 LLM 후보 재정렬은 기본 비활성화이고, HTML 근거로 충분하면 PDF·OCR을 시작하지 않습니다.
...
- `POST /api/feedback`
- `GET /api/notices/{notice_id}/media/{media_index}`
- `GET /api/notices/{notice_id}`
- `GET /api/health`

`X-Admin-Token`이 필요한 운영 API:

- `POST /api/crawler/run` (`X-Admin-Token` 필요)
- `GET /api/crawler/preview` (`X-Admin-Token` 필요)
- `GET /api/crawler/status` (`X-Admin-Token` 필요)
- `POST /api/index/rebuild`, `GET /api/index/rebuild/preview`
- `GET /api/data-gaps`, `PATCH /api/data-gaps/{gap_id}`
- `POST /api/notices/{notice_id}/reprocess` (`X-Admin-Token` 필요)
- `POST /api/notices/{notice_id}/codex-queue`
- `/api/enrichment/*`, `/api/codex/*`, `/api/on-demand-codex/*` 작업 큐 API
...
현재 기준 백엔드 216개, 프런트엔드 22개 테스트를 포함합니다. 변경 감지, Codex/OpenAI 구조화 큐, 온디맨드 캐시, 누락 근거 복구, 확장 사실, 액션 가이드 교체·링크 검증, 하이브리드 검색, 데이터 없음 응답, 개인정보 입력 차단, 관리자 API 보호, 피드백 저장, 크롤러 파서, API와 주요 UI 흐름을 검증합니다. GitHub Actions도 push와 Pull Request마다 백엔드·프런트엔드·빌드·비밀정보 검사를 실행합니다.

## 개발 기록과 Git 사용법

이 저장소는 커밋을 포트폴리오 기록으로 활용합니다. `.gitmessage`와 Pull Request 템플릿이 `문제 → 원인 → 해결 → 검증 → 효과` 순서를 안내합니다.

```bash
git config --local commit.template .gitmessage
```

커밋 분리 기준, 기술 교체 시 함께 기록할 파일, 검증 명령은 [기여 및 개발 기록 규칙](CONTRIBUTING.md)을 따릅니다. 주요 문제 해결 사례는 [개발 문제 해결 기록](docs/development-log.md)에 누적합니다.
